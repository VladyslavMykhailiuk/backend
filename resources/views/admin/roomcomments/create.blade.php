...
@section('section')
    <div class="d-flex h-100">
        <form class="m-auto p-5" action="{{route('admin.roomcomments.store')}}" method="post" enctype="multipart/form-data" >
            @csrf
            <div class="form-group">
                ...
                <select name="room_id" class="form-control" id="room_id">
                    ...
                    @foreach($rooms as $id => $name)
                        <option value="{{ $id }}" {{ old('room_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
